...
| Git `.git/objects` 膨胀 | `.git` 与 worktree 同挂载时，每次 fetch/commit 都会在 JuiceFS `jfs_node` 等表产生大量 inode；见 **4.4.1** |
| 噪声邻居                  | 大租户读写可能影响他人，需配额、限速与监控                                                                  |
| 误删/越权                 | 业务层 ACL 必须在 Workspace Server 控制面完成，不依赖文件系统权限                                           |


#### 4.4.1 Git 元数据与 JuiceFS 分离

##### 问题

JuiceFS 会为挂载点上的**每一个文件/目录**维护原生元数据（MySQL 中的 `jfs_node`、`jfs_edge`、`jfs_chunk` 等）。这与业务侧的 `file_node` / `jfs_wedata_identity` **无关**——即使 IdentityFS 跳过 `.git/`** 不写身份表，只要 `.git` 目录在 JuiceFS 上，go-git / 原生 `git` 的 objects、refs、index 变更仍会写入 `jfs_node`。

Git 操作（clone、fetch、pull、commit、gc）会高频、大量地创建/更新 `.git/objects/**` 下的小文件，导致：

- 共享 JuiceFS 元数据库 inode 总量快速上升；
- 大仓 clone 时元数据写入成为瓶颈；
- 与「用户可见 worktree 文件」混在一起，难以按业务维度做容量规划。

##### 目标拓扑

**worktree（用户可见）** 留在 JuiceFS；**Git object database（`.git`）** 放在 JuiceFS 之外的本地盘或独立卷（可按节点、可用区或专用元数据盘部署）。

```text
JuiceFS mount (workspace.mount_root)
  {tenantId}/{workspaceId}/users/{userUin}/{repoName}/   # 仅工作区文件，无 .git

Git 元数据根 (workspace.git_meta_root，非 JuiceFS)
  {tenantId}/{workspaceId}/users/{userUin}/{repoName}/.git/   # objects、refs、config...
```

go-git 已支持 **Storage 与 Worktree 分离**（与 6.4.2 示例一致）：

```go
gitDir := resolveGitDir(mountRoot, gitMetaRoot, worktreePath) // 例如 /var/wedata/git-meta/.../repo/.git
gitStorage := filesystem.NewStorage(osfs.New(gitDir), cache.NewObjectLRUDefault())
worktreeFS := NewIdentityFS(ctx, osfs.New(worktreePath), actor, identityStore)
repo, err := git.CloneContext(ctx, gitStorage, worktreeFS, cloneOpts)
```

路径映射规则（与 workspace-service 实现一致）：

- `worktreePath` 必须为 `mount_root` 下的绝对路径；
- `rel = relpath(mount_root, worktreePath)`，例如 `users/200001/myrepo`；
- `gitDir = join(git_meta_root, rel, ".git")`；
- `DeleteRepo` 需同时删除 worktree 与外部 `gitDir`（若二者不在同一目录树下）。
...
| 配置项                       | 说明                                               |
| ------------------------- | ------------------------------------------------ |
| `workspace.mount_root`    | JuiceFS 挂载根（worktree）                            |
| `workspace.git_meta_root` | Git 元数据根目录；**为空时** `.git` 落在 worktree 下（仅适合本地开发） |


生产建议：

1. **Workspace Server Pod**：除挂载 JuiceFS 外，再挂载 `git_meta_root`（hostPath、emptyDir+本地 SSD、或独立云盘），并保证与 worktree 路径映射稳定。
2. **DLC driver / Terminal Pod**：若用户在 shell 内直接执行 `git`，同样需把 `GIT_DIR` 指到外部元数据路径，或统一走 Workspace Server Git API；否则仍会写回 JuiceFS 上的 `.git`（见 6.4.7）。
3. **权限**：`git_meta_root` 按 `owner_uin` / `uin` 子目录隔离，与 worktree 路径规则对齐；禁止业务 API 通过文件接口读写 `git_meta_root`。
4. **容量**：监控 `git_meta_root` 磁盘与 inode；可配合 `git gc`、shallow clone、单分支 clone 控制对象体积。

##### CloneRepo 与元数据体积

`CloneRepo` 要求调用方传入 `**branch`**，底层使用 go-git：

- `SingleBranch: true` — 只拉取指定分支历史；
- `Tags: NoTags` — 不拉取 tags，避免 tag 指向的 object 拖垮 clone 体积。

仍建议对大仓评估 `**Depth`（shallow clone）** 与 partial clone（go-git 能力演进后再开）；二者进一步降低 `git_meta_root` 与网络开销，与「`.git` 出 JuiceFS」正交、可叠加。

##### 与身份索引的关系


| 存储                                  | `.git/`**                    | worktree 业务文件 |
| ----------------------------------- | ---------------------------- | ------------- |
| JuiceFS `jfs_node`                  | **不应写入**（`.git` 不在 mount 上）  | 写入            |
| `jfs_wedata_identity` / `file_node` | 不记录（IdentityFS `shouldSkip`） | 记录            |
